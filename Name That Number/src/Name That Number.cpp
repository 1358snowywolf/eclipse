...
public:
	vector<string> makeName(string serial_number, const string vtl[][3], unordered_set<string> correct_names){
		...
		string name = "";
		transform(serial_number, name, vtl, correct_names);
...
private:
	void transform(string serial_number, string name, const string vtl[][3], unordered_set<string>& correct_names){
		if((int)serial_number.size() <= 0){
			if(correct_names.find(name) != correct_names.end()){
				names.insert(name);
			}
//			names.insert(name);
			return;
		...
		int digit = serial_number[(int)serial_number.length()-1] - '0';
